...
int detector::YoloV8::init(std::string engine_path, std::string plugin_path) {
    ...
    auto checkCuda = [](cudaError_t status, const char* action) {
        if (status != cudaSuccess) {
            std::cerr << "CUDA error(" << action << "): " << cudaGetErrorString(status) << std::endl;
            return false;
        }
        return true;
    };
    ...
    if (!checkCuda(cudaMalloc(reinterpret_cast<void**>(&cuda_outputs), output_size * sizeof(float)),
                   "cudaMalloc(outputs)")) {
        return -1;
    }
...
bool detector::YoloV8::resolveTensorNames() {
    if (!engine) {
        std::cerr << "Error: Engine is not ready when resolving tensor names" << std::endl;
        return false;
    }
    ...
    if (input_names.size() != 1 || output_names.empty()) {
        std::cerr << "Error: Expected exactly 1 input tensor and at least 1 output tensor, but got inputs="
                  << input_names.size() << " outputs=" << output_names.size() << std::endl;
        return false;
    }
...
detector::YoloV8::infer(std::vector<cv::Mat>& raw_image_generator) {
    ...
    if (!context->setInputShape(input_tensor_name_.c_str(), input_dims)) {
        ...
        return std::make_tuple(batch_image_raw, 0.0, std::vector<std::vector<float>>{}, std::vector<std::vector<float>>{}, std::vector<std::vector<int>>{}, std::vector<std::vector<float>>{});
    }
