...
public:
     ...
     * @param flag 图像类型 True: car False: armor
    ...
    std::tuple<
        ...
        std::vector<std::vector<float>>>
    infer(std::vector<cv::Mat>& raw_image_generator, bool flag);
...
private:
    ...
    post_process(float* output, int origin_w, int origin_h, int output_size, bool flag);
